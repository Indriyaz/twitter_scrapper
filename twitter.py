...
with st.sidebar:
    # Logo in sidebar.
    image_url = ("https://rilion.in/wp-content/uploads/2023/01/Twitter-Scraper.png")
...
# as scraper allows query in a predefined format.
    search = keyword + f" since:{start_date}"+f" until:{end_date}"

# Input from the user for number of tweets to scrape.
    no_of_tweets = (st.text_input("Enter the number of tweets"))
# For naming the file.
file_name = (keyword + f" {timestamp}" + f" {no_of_tweets}" + f" scraped data from {start_date} to {end_date}")
...
# Gif for body of the streamlit app.
image_url= ("https://rilion.in/wp-content/uploads/2023/01/Twitter-scraper.gif")
st.image(image_url,use_column_width="auto")
